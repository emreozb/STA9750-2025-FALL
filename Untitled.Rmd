...
project:
  ...
  output-dir: docs
website:
  title: "YIMBY by Numbers: Identifying America’s Most Housing-Friendly Cities"
  navbar:
    search: true
  pdf:
    toc: true
...
#   year int
# 
#   Note: 'ACS 1-year, 2009–2023 (no 2020)'
#   Indexes {
#     (GEOID, year) [pk]
#   }
# }
...
# // ---------- Relationships ----------
# 
# // ACS tables are different measures of the SAME CBSA–year.
...
# 
# // Permits are also CBSA–year; join by CBSA = GEOID + year.
...
### 🧭 What the Arrows Mean (Cardinality)
...
#### **PERMITS → (ACS Tables)**
**1-to-1 per (`CBSA`, `year`) → (`GEOID`, `year`)**  
...
top10_permits <- PERMITS %>%
    ...
    dplyr::transmute(
        `CBSA Name` = NAME,
        `CBSA Code` = CBSA,
        `Total Permits (2010–2019)` = total_permits_2010_2019
    )
...
datatable(
    ...
    caption = htmltools::tags$caption(
        ...
        'Top 10 CBSAs by Total Housing Permits (2010–2019)'
    )
)
```
...
This reflects D.C.’s concentration of high-wage federal and professional jobs, an educated workforce, and a higher cost of living.
:::
...
datatable(
  ...
  caption = "Top 5 Years: Finance & Insurance Share of Total Wages — NYC CBSA (NAICS 52)"
) |>
...
ggplot(employment_trend, aes(x = total_employment, y = healthcare_employment, color = YEAR)) +
  ...
  labs(
    title = "Evolution of Health Care & Social Assistance Employment vs Total Employment (2009–2023)",
...
ggplot(employment_trend, aes(x = YEAR, y = share_healthcare)) +
  ...
  labs(
    title = "Average Share of Health Care Employment over Time (2009–2023)",
    x = "Year",
...
### 🏙️ Rent Burden Across U.S. Metropolitan Areas
...
### 🗽 New York’s Rent Burden Over Time
...
### 📈 National Interpretation and Broader Context
...
### 🏗️ Building-Friendliness Insights
...
#  - also create percentile-like index (0–100) for interpretability
...
datatable(
  rent_burden_rank %>%
    ...
    rename(
      ...
      `Rent Burden Index (0–100)` = rent_burden_index
    ),
  ...
  formatRound('Rent Burden Index (0–100)', 1)
```
...
# 3) Standardize to 0–100 scales for interpretability
...
datatable(
  bind_rows(top_build, bottom_build) %>%
    ...
    rename(
      ...
      `Building-Friendliness Index (0–100)` = build_index,
      `Permit Growth – Population Growth` = growth_diff
    ),
  caption = paste0("CBSAs with Highest and Lowest Building-Friendliness (", latest_year, ")"),
  options = list(dom = 't', paging = FALSE, scrollY = 400)
) %>%
  formatRound(c('Permits per 1,000 residents', 'Building-Friendliness Index (0–100)', 'Permit Growth – Population Growth'), 2)
```
...
::: {.callout-note title="Key Takeaways"}
### 🏠 Key Takeaways on Housing Growth (2023)
**1.Top-Performing Metro Areas**
In 2023, **Salisbury, MD, Myrtle Beach–Conway–North Myrtle Beach, SC, and Punta Gorda, FL** led the nation in instantaneous housing growth.
- **Salisbury** stands out with a Building-Friendliness Index of **100, reflecting 37.7 permits per 1,000 residents**—the highest in the dataset despite its negative 5-year population growth.
...
- Southern and coastal regions dominate the top rankings—especially Florida and South Carolina—showing strong residential expansion in smaller or mid-sized metros.
...
- The **composite index** provides a more nuanced view—identifying areas that balance immediate building activity with sustainable long-term growth.
:::
...
fmt_dt <- function(dtobj) {
  dtobj |>
    ...
    # your "5-yr Pop Growth (%)" is already on a 0–100 scale, so use formatRound (not formatPercentage)
...
datatable(
  ...
  caption  = paste0("Instantaneous Housing Growth — Top 10 (", last_year, ")")
) |> fmt_dt()
```
...
datatable(
  ...
  rownames = FALSE,
  options  = list(dom = 't', paging = FALSE),
  caption  = paste0("Instantaneous Housing Growth — Bottom 10 (", last_year, ")")
) |> fmt_dt()
```
### Visualization: The Relationships between Rent Burden and Housing Growth

::: {.callout-note title="Key Takeaways"}
### 🧩 Insights: Rent Burden & Housing Growth (2009–2023)
**1) Scatter Plot — Rent Burden Change vs Housing Growth**
...
- Population-growth coloring reveals that **metros with strong population gains** (yellow points) often experienced only small rent-burden increases or mild declines → evidence that new construction helped absorb growth.
- The “YIMBY success zone” lies in the **lower-right quadrant** (high growth + falling burden), occupied by cities such as Gulfport-Biloxi (MS) and Auburn-Opelika (AL).

**2) Time Series — Rent Burden Trends in High-Housing-Growth CBSAs**
- Across high-growth metros, **the average rent-burden index remained flat or declined slightly** between 2009 and 2023.
- Despite short-term fluctuations (notably around 2020–2021), the **aggregate blue line** shows that expanding housing supply correlated with **stabilizing affordability** rather than runaway rent escalation.
- This contrasts with slower-building metros, where burden indices rose steadily.

**3) Top 10 ‘YIMBY Success’ CBSAs (Table)**
**Avg Housing Growth Index:** Highest in Myrtle Beach (87.8) and Hilton Head Island (55.5) – both sustained large permitting rates.
...
**Overall YIMBY Score:**Leaders combine high housing growth + declining burden + population gain → balanced, not declining metros.

**3) Bar Chart — Rent Burden Reduction in Top Cities**
...
- Others like Tallahassee (FL) and Valdosta (GA) maintained moderate reductions, suggesting steady—but not excessive—building activity.
...
ggplot(viz_data, aes(x = avg_housing_growth, y = rent_burden_change, color = total_pop_growth)) +
  ...
  labs(
    ...
    y = "Change in Rent Burden Index (Final − Initial)"
  ) +
  theme_minimal(base_size = 13)
...
rent_burden %>%
  ...
  labs(
    ...
    subtitle = "Blue line shows average rent burden trajectory (2009–2023)",
    x = "Year",
...
# Early (2009–2012) vs Recent (2021–2023) rent-burden index, by CBSA
rb_period <- rent_burden %>%
  mutate(period = case_when(
    year %in% 2009:2012 ~ "Early (2009–2012)",
    year %in% 2021:2023 ~ "Recent (2021–2023)",
    TRUE ~ NA_character_
  )) %>%
  filter(!is.na(period)) %>%
...
viz_base <- rb_wide %>%
  ...
  rename(early  = `Early (2009–2012)`,
         recent = `Recent (2021–2023)`)
...
ggplot(yimby_plot_set) +
  ...
  labs(
    ...
    x = "Rent Burden Index (0–100)", y = NULL,
    caption = "Red = Early (2009–2012) • Green = Recent (2021–2023) • Dotted line = national early average"
  ) +
  theme_minimal(base_size = 13) +
  theme(
