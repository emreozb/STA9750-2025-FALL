...
![](figs/intro_header2.png)

## Introduction
Central Park stands out in green at the heart of Manhattan, but **1,093,439** trees are spread across all five of New York City’s boroughs. Together, they form one of the most diverse and essential urban forests in the United States. These trees cool neighborhoods in summer, improve air quality, reduce flooding, and contribute to the character and livability of every blockn from the dense streets of Midtown to the quieter corners of Staten Island.

This mini project explores NYC’s green canopy using spatial data from the city’s most recent tree census. By visualizing tree health, distribution, and species across council districts, we aim to understand how well the city’s urban forest is being maintained and where targeted interventions may be needed. The analysis highlights both strengths—such as robust clusters of healthy trees and challenges like uneven maintenance, aging tree populations, or districts with disproportionate counts of poor-condition trees.
...
### 🗺️ T1: NYC City Council District Boundaries
...
### ️🌳  T2: NYC Street Tree Census (Primary Dataset)
...
- **Staten Island dominates** tree coverage — it holds **3 of the top 6 districts**, including **District 51**, which has by far the most trees (**70,927**), significantly ahead of all others.
...
trees_density_km2 <- trees_with_cd |>
  ...
  mutate(
    area_sqkm    = Shape_Area / 1e6,                 # convert sq meters → sq km
...
- Most top districts fall within a **narrow range (13%–14%)**, showing a **citywide, consistent level of dead-tree burden** rather than extreme outliers.
...
cond_by_cd <- trees_with_cd |>
  st_drop_geometry() |>
  count(CounDist, tpcondition, name = "n") |>
  group_by(CounDist) |>
  mutate(
    total = sum(n),               # <-- THIS is the total column
    pct   = 100 * n / total
  ) |>
  ungroup()
...
dead_top10 <- cond_by_cd |>
  ...
  mutate(
    Rank      = row_number(),
    total_fmt = format(total, big.mark = ","),  # <-- changed here
...
::: {.callout-note title="Findings"}
- **Thornless honeylocust** is by far the most common street tree in Manhattan, making up **13.3%** of all trees—well ahead of the second-place species.

- The next most common species **London planetree, Callery pear**, and **pin oak** range from **6–9%**, showing a more balanced distribution after the top species.

- **Ginkgo trees** also represent a notable share (**5.7%**), reflecting Manhattan’s long use of ginkgos in high-density urban corridors.
...
::: {.callout-note title="Findings"}
- **🌳 Closest Tree Species:** The tree nearest to Baruch College is a **Quercus acutissima (Sawtooth Oak)**.
...
cat("🌳 The tree closest to Baruch College is:", species_clean, "\n")
```
...
## T5: 🌳 NYC Parks Proposal: District Tree Restoration & Renewal Initiative
...
Council District 32 faces a growing urban forestry challenge: a disproportionately high number of **dead and declining street trees** that pose safety risks, degrade neighborhood appearance, and reduce the district’s overall canopy health. We propose a targeted **Dead Tree Removal & Replanting Initiative** to eliminate hazardous dead trees, grind existing stumps, and replant new, climate-resilient species in priority locations across the district.
...
- **Plant 2,000–3,000 new trees** over two planting cycles
...
Council District 32 stands out in New York City not because of its size, but because of the **scale of environmental risk posed by dead street trees.** According to the latest NYC Street Tree Census, District 32 has the **highest share of dead trees in the entire city, with over 14% of its street trees recorded as dead**—a rate significantly above the citywide average and the highest among all 51 districts.

This is not a marginal issue. It represents **4,305 dead trees** across the district’s neighborhoods—from Lindenwood and Howard Beach in the north to Broad Channel, Rockaway Park, and the Rockaway Peninsula in the south. These are trees that no longer provide shade, environmental cooling, or air-quality benefits—and more importantly, they now pose real safety risks to residents, drivers, cyclists, and pedestrians.

#### 🔎 A Tale of Two Clusters: North and South
...
The southern section, spanning Broad Channel and the Rockaway Peninsula, reveals a **continuous corridor of dead street trees**—one of the longest such concentrations in the entire city. This area is particularly vulnerable because:
...
This is not just an environmental concern—it is a **public safety issue.**
...
### 🚨 Why This Matters: District 32 Is Out of Balance

The bar chart makes the picture clear: District 32 is not simply “near the top” of any list—it is **the top district in NYC** for dead trees by percentage.
...
### 🌱 What NYC Parks Can Do — and How This Proposal Helps
...
Using NYC Parks’ forestry systems, these hotspot corridors should be placed on enhanced monitoring cycles.
...
### 🌆 Conclusion: A Clear Opportunity for Impact
...
A targeted, data-driven investment in this district would not only address current hazards but would also **rebuild the neighborhood canopy for future generations**—exactly the kind of high-impact work NYC Parks is best positioned to lead.
...
set.seed(123)
# 1) Filter District 32 first
trees_32 <- trees_with_cd %>%
  filter(CounDist == 32)

# 2) Sample up to 10,000 trees (or all of them if there are fewer)
trees_sample <- trees_32 %>%
  slice_sample(n = min(10000, nrow(trees_32)))
...
I also downloaded the Tree Risk Assessment dataset, which includes risk ratings such as ‘Low’, ‘Moderate’, ‘Moderate to High’, and ‘Critical’. I used this data to identify how many risky trees exist in District 32 and to compare risk levels to the percentage of dead trees.
